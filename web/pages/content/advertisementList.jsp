...
<cartmatic:cartmaticBtn btnType="cancel" inputType="button"
	onclick="return fnDoUpToParent(this);" />
</content>
<app:ui_leftMenu>
	<div class="sidebar-left">
		<form method="post" action="${ctxPath}/content/advertisement.html"
			...
			<div class="tab" id="left_menu_tabs">
				...
				<div class="content" id="listSelectContent">
					<search:searchBox>
						<search:filter />
						<search:input attrPath="s.advertisementName"
							attrNameKey="advertisement.advertisementName" datatype="String"
							operator="LIKE" classes="form-inputbox" />
						<div class="title">
							<fmt:message key="adPositionType.store" />
						</div>
						<div>
							<select name="COL@s.adPositionType.store.storeId@Integer@EQ"
								id="storeId" style="width: 150px">
								<option value=""></option>
								<c:forEach items="${appConfig.storeMap}" var="storeItem">
									<option value="${storeItem.value.storeId}"
										<c:if test="${param['COL@s.adPositionType.store.storeId@Integer@EQ'] == storeItem.value.storeId}">selected="selected" </c:if>>
										${storeItem.value.name}
									</option>
								</c:forEach>
							</select>
...
<form class="mainForm" name="advertisementListForm" method="post"
	action="${ctxPath}/content/advertisement.html">
	<!--listing box start-->
	<!--editURLPath is used in TblDecorator-->
	<c:set var="editURLPath"
		value="/content/advertisement.html?doAction=edit&from=list"
		scope="page" />
	<c:set var="checkAll">
...
class="checkbox" />
	...
	<display:table name="${advertisementList}" cellspacing="0"
		cellpadding="0" uid="advertisementItem" class="table-list"
		style="table-layout:fixed;" export="false" requestURI="">
		<display:column headerClass="w30" class="w30" title="${checkAll}"
			media="html">
			<input type="checkbox" name="multiIds"
				value="${advertisementItem.advertisementId}" class="checkbox"
				title="${advertisementItem.advertisementName}" />
		</display:column>

		<display:column headerClass="w100" class="w100" sortable="false"
			decorator="com.cartmatic.estore.core.decorator.TblColumnDecorator"
			titleKey="advertisement.view" media="html">
			<div
				onmouseout="$j('#preview${advertisementItem.advertisementId}').hide();">
				<a href="javascript:;"
					onclick="$j('#preview${advertisementItem.advertisementId}').show();"><fmt:message
						key="advertisement.view" /> </a>
			</div>
			<div style="position: absolute; display: none; z-index: 150;"
				id="preview${advertisementItem.advertisementId}">
				<c:choose>
					<c:when test="${advertisementItem.contentType==2}">
						<OBJECT height="90" width="550"
		...
		<display:column headerClass="wauto" class="wauto" sortable="false"
			url="${editURLPath}" paramId="advertisementId"
			paramProperty="advertisementId"
			decorator="com.cartmatic.estore.core.decorator.TblColumnDecorator"
			titleKey="advertisement.advertisementName">
		...
		<display:column sortable="false" paramId="culturalInformationId"
		paramProperty="culturalInformationId" title="图片压缩图"
		decorator="com.cartmatic.estore.core.decorator.TblColumnDecorator"
		titleKey="culturalInformation.culturalInformationName">
		<cartmatic:img isUrlOnly="false" url="${advertisementItem.url}"
			mediaType="a_and_d" id="logoImage" height="200" width="300"></cartmatic:img>
	     </display:column>
	     
		<display:column property="contextTitle" sortable="true"
			headerClass="data-table-title" title="广告上文字主标题"
			decorator="com.cartmatic.estore.core.decorator.TblColumnDecorator" />
