...
<c:choose>
	<c:when test="${not empty multiSelect&&multiSelect}">
		...
		if(val){
				...
				var supplier=__fnGetObjJsonData_${id}(pid);
				fnAddSelectedSupplier${id}(supplier);
				$j("#sel_ch_${id}_"+supplier.culturalInformationId).attr("checked",true);
			}
			
		}
		function fnAddSelectedSupplier${id}(supplier){
		  // alert("双击添加");
		  // alert("supplierID:"+supplier.culturalInformationId)
			var exist=false;
			for(var i=0;i<selectedSupplierList${id}.length;i++){
				if(selectedSupplierList${id}[i].supplierId==supplier.culturalInformationId)
					exist=true;
			}
		//	alert("exist:"+exist);
			if(!exist){
