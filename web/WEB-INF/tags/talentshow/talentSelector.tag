...
<%@ attribute name="culId" required="false"  rtexprvalue="true" type="java.lang.String" description="达 人秀ID"%>
...
function ${id}_show(paramData)
{
	...
	if(!(supplierSelector_dialog_${id}.dialog("isOpen")==true||supplierSelector_dialog_${id}.dialog("isOpen")==false)){
		...
		fillDivWithPage("supplierSelector_divDlg${id}",__ctxPath+"/selector/talentShowSelector.html?culId=${culId}",getSupplierSelectorData_${id}(),"post");
...
function fn${id}GetData(){
	...
	fillDivWithPage("supplierSelectorDataList_${id}","${ctxPath}/selector/talentShowSelectorDataList.html?pagingId=${id}&culId=${culId}",paraDate${getDataCallBack},"post");
}
...
<c:choose>
	<c:when test="${not empty multiSelect&&multiSelect}">
		...
		if(val){
				...
				var supplier=__fnGetObjJsonData_${id}(pid);
				fnAddSelectedSupplier${id}(supplier);
				$j("#sel_ch_${id}_"+supplier.talentShowId).attr("checked",true);
			}
			
		}
		function fnAddSelectedSupplier${id}(supplier){
		   // alert("双击添加");
		   // alert("supplierID:"+supplier.talentShowId)
			var exist=false;
			for(var i=0;i<selectedSupplierList${id}.length;i++){
				if(selectedSupplierList${id}[i].supplierId==supplier.talentShowId)
					exist=true;
			}
		 //	alert("exist:"+exist);
			if(!exist){
			 //  alert("exit");
				...
				$j("#selectedSupplier_${id}").append("<span id='sel_${id}_"+supplier.talentShowId+"' ondblclick='fnRemoveSelectedSupplier${id}("+supplier.talentShowId+")' title='双击移除'>"+supplier.content+",</span>");
			}
		}
		...
		function fnRemoveSelectedSupplier${id}(id){
		   // alert("移除所选:"+id);
		//	for(var i=0;i<selectedSupplierList${id}.length;i++){
			//	if(selectedSupplierList${id}[i].talentShowId==id){
