...
/**
 * CartCategoryPercentPointDonateAction donate point by the
 * category(key=CATEGORY) percent (key=DONATE_PERCENT)
 * 
 * @author CartMatic
 * 
 */
public class CartCategoryPercentPointDonateAction extends CartAbstractAction {
	private static final Log	logger			= LogFactory
														.getLog(CartCategoryPercentPointDonateAction.class);
	...
	public CartCategoryPercentPointDonateAction(Map<String, String> _params)
			throws Exception {
		...
		if (paramDonatePercent.compareTo(new BigDecimal(0)) == -1
			...
			throw new Exception(
					"CartCategoryPercentPointDonateAction--error percent,it should between 0 to 100");
		} else {
	...
	public Shoppingcart run(Shoppingcart _cart) {
		...
		logger.debug(new StringBuffer().append("[DONATE_PERCENT|").append(
				paramDonatePercent).append("]").append("[CATEGORY|").append(
