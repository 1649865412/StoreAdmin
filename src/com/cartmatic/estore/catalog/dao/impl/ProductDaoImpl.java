...
public class ProductDaoImpl extends HibernateGenericDaoImpl<Product> implements ProductDao
{
	...
	public List<Product> getDefaultProduct(String sort) {
		/*
		 * Map<String, Object> paramValue =new HashMap(); List<Product> result =
		 * getObjectList(SqlSearch,new Product(),paramValue); return result;
		 */
		...
		try
		{
			if (sort.equals("up"))
			{
				result = findByHql("from Product  a where a.status = 1 order by  a.defaultProductSku.price asc");
			}
			else if (sort.equals("down"))
			{
				result = findByHql("from Product  a where a.status = 1 order by  a.defaultProductSku.price desc");
			}
		}
		catch (Exception e)
		{
			result = findByHql("  from Product a  where a.status=1 ORDER BY  a.createTime asc");
		}
		return result;
	}
	 ...
	 * @return
	 */
	private SearchCriteria searchProducts(SearchCriteria searchCriteria, ProductSearchCriteria productSearchCriteria,
			String defaultSelectObject, String defaultWhereClause, String orderBy) {
		...
		if (StringUtils.isNotEmpty(productSearchCriteria.getName()))
		{
			...
			/*
			 * whereClause=getAndClause(whereClause,
			 * "LOWER(p.productName) like ?");
			 * paramList.add("%"+productSearchCriteria
			 * .getName().toLowerCase()+"%");
			 */
		}
		...
		if (StringUtils.isNotEmpty(productSearchCriteria.getCategoryPath())
				&& !productSearchCriteria.getCategoryPath().equals(Constants.ROOT_CATEGORY_CATALOG.toString()))
		{
			fromClause = getFromClause(fromClause, "ProductCategory pc");
			...
			for (int i = 0; i < searchAttributeList.size(); i++)
			{
				...
				if (i == 0)
				{
					...
					for (int j = 0; j < searchAttributeItemList.size(); j++)
					{
						...
						String opertorAndValue[] = SearchUtil.getOperatorAndValue(searchAttributeItem.getOperator(), searchAttributeItem
								.getValue());
	...
	public List<Product> getByBrandId(Integer brandId){
		List<Product> result = new ArrayList();
		result = findByHql("from Product a where a.brand.brandId = ? order by  a.createTime desc",brandId);
		return result;
	}
