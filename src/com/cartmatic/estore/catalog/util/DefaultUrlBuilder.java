...
public class DefaultUrlBuilder implements UrlBuilder{
	...
	public String getContentUrl(Content content) {
		...
		List<Category>navigatorCategorys=category.getNavigatorCategorys();
		for (Category category2 : navigatorCategorys) {
			if(ConfigUtil.getInstance().getCmsAccessCategories().indexOf(","+category2.getCategoryCode()+",")!=-1){
				url_prefix="/"+category2.getCategoryCode()+"/";
				break;
			}
		}
		StringBuffer url=new StringBuffer(url_prefix);
		//url.append(UrlUtil.formatUriPart(content.getContentTitle().toLowerCase()));
		url.append("_");
