...
public class SchedulerUtil {
	...
	private final static Log	logger		= LogFactory
													.getLog(SchedulerUtil.class);
	public final static String	PAUSED_FLAG	= "[P]";
	private static String		xmlFile		= ContextUtil
													.getServletContext()
													.getRealPath(
															"/WEB-INF/classes/conf/spring-scheduling.xml");

	/**
	 * 记录出错的Trigger TODO 可以发送Email
	 * 
	 * @param triggerName
	 * @param exception
	 */
	public static void addErrorJob(String triggerName,
	...
	public static void pausedTriggerInConfig(String triggerName)
			throws Exception {
		Document document = getConfigDocument();
		Node node_desc = document.selectSingleNode("/beans/bean[@id='"
				+ triggerName + "']/property[@name='description']/@value");
	 ...
	 */
	public static void startupScheduler(Scheduler scheduler)
		...
		for (int i = 0; i < triggerGroups.length; i++) {
			String[] triggersInGroup = scheduler
					.getTriggerNames(triggerGroups[i]);
			for (int j = 0; j < triggersInGroup.length; j++) {
				Trigger t = scheduler.getTrigger(triggersInGroup[j],
						triggerGroups[i]);
				if (t.getDescription() == null
						|| !t.getDescription().endsWith(PAUSED_FLAG)) {
					scheduler.resumeTrigger(triggersInGroup[j],
							triggerGroups[i]);
				}
			}
		}
	...
	public static void unpausedTriggerInConfig(String triggerName)
			throws Exception {
		Document document = getConfigDocument();
		Node node_desc = document.selectSingleNode("/beans/bean[@id='"
				+ triggerName + "']/property[@name='description']/@value");
	 ...
	 * @throws Exception
	 */
	public static void updateCronTriggerConfig(String triggerName,
			String cronExpression) throws Exception {
		Document document = getConfigDocument();
		Node node = document.selectSingleNode("/beans/bean[@id='" + triggerName
				+ "']/property/value");
	 ...
	 * @throws Exception
	 */
	public static void updateSimpleTriggerConfig(String triggerName,
			long repeatInterval) throws Exception {
		Document document = getConfigDocument();
		Node node_repeatInterval = document
				.selectSingleNode("/beans/bean[@id='" + triggerName
