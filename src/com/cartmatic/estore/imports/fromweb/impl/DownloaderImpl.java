...
public class DownloaderImpl implements Downloader
{
	...
	public Reader download(String urlPath, InputStream httpIs, boolean isCompressionInput)throws IOException
	{
		...
		if (!Downloader.LIB_PRODUCT_URL_PAT.matcher(urlPath).matches())
		{
	...
	private void downloadProduct(String htmlContent) throws Exception
	{
		...
		NodeList nodes = null;
		//产品名
		nodes = parser.extractAllNodesThatMatch(new HasAttributeFilter("class","fl pad_product line_180"));
		TextNode pname = (TextNode)nodes.elementAt(0).getChildren().elementAt(1).getFirstChild();
		String productName = pname.getText();
		//产品编码 & SKU 目前只使用UUID实现.
		String productCode = java.util.UUID.randomUUID().toString();
		//主目录
		parser.reset();
		nodes = parser.extractAllNodesThatMatch(new HasAttributeFilter("id","nav_bread_crumb"));
		NodeList cats = nodes.elementAt(0).getChildren().elementAt(1).getChildren();
		String catalogName = "";
			...
			if (cats.elementAt(i) instanceof LinkTag)
			{
				...
				if (catalogName.length() == 0)
				{
					if (!cat.getLinkText().equals("Home"))
						catalogName = cat.getLinkText().substring(9).trim();
				}
				else
				{
					catalogName = catalogName + ">"+cat.getLinkText().substring(9).trim();
				}
			}
		}
		//meta关键字
		...
		//产品描述
		parser.reset();
		nodes = parser.extractAllNodesThatMatch(new HasAttributeFilter("id","Item_Description_Spc"));
		String productDesc = nodes.elementAt(0).getChildren().toHtml().replaceAll("http://www.lightinthebox.com", "#");
		//价格
		parser.reset();
		nodes = parser.extractAllNodesThatMatch(new HasAttributeFilter("id","products_price_unit"));
		TextNode price = (TextNode)nodes.elementAt(0).getFirstChild();
		String productPrice = price.getText();
		//产品图片&下载图片到本地
		parser.reset();
		nodes = parser.extractAllNodesThatMatch(new HasAttributeFilter("id","product_flash_show"));
		ImageTag img = (ImageTag)nodes.elementAt(0).getFirstChild();
