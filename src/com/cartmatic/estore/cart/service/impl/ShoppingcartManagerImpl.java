...
import com.cartmatic.estore.juankong.UrlText;
...
public class ShoppingcartManagerImpl extends GenericManagerImpl<Shoppingcart> implements
		com.cartmatic.estore.cart.service.ShoppingcartManager
{
	...
	public void setShoppingcartItemGcManager(ShoppingcartItemGcManager shoppingcartItemGcManager) {
	 ...
	 * if (logger.isDebugEnabled()) {
	 * logger.debug("enter isExpiration method......"); } Date dateNow = new
	 * Date(); Date expirationDate =
	 * DateUtil.changeDate(shoppingcart.getUpdateTime(), 0, 0, validDays, 0, 0,
	 * 0).getTime(); return dateNow.after(expirationDate); }
	 */

	/**
	 * 叠加购物车产品数量
	 */
	private ShoppingcartItem plusItemNo(Shoppingcart cart, ShoppingcartItem newItem, Integer quantity) throws Exception {
		...
		for (ShoppingcartItem item : pset)
		{
			if (!item.getItemType().equals(Constants.ITEM_TYPE_PRODUCT))
				continue;
			if (item.getProductSku().getProductSkuCode().equals(newItem.getProductSku().getProductSkuCode())
					&& ((item.getParent() == null && newItem.getParent() == null) || (item.getParent().getProductSku().getProductSkuCode()
							.equals(newItem.getParent().getProductSku().getProductSkuCode()))))
			{
	...
	public Shoppingcart addMultiProductToCart(String cartUuid, String parentSku, String[] childrenSkus, Integer quantity,
			String accessoryIds, short isSaved, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ProductSku sku = productService.getProductSkuByProductSkuCode(parentSku);
		ShoppingcartItem newCartItem_p = shoppingcartItemManager.newShoppingcartItem(sku, quantity, accessoryIds, isSaved);
		newCartItem_p = doAdd(cart, newCartItem_p, quantity);
		for (int i = 0; i < childrenSkus.length; i++)
		{
			ProductSku sku_p = productService.getProductSkuByProductSkuCode(childrenSkus[i]);
			if (sku_p.getIsFictitious())
			{
				ShoppingcartItem newCartItem_c = shoppingcartItemManager.newShoppingcartItem(sku_p, quantity, accessoryIds, isSaved);
	...
	private Shoppingcart addProduct(String cartUuid, String productsku, Integer quantity, String accessoryIds, String customMade,
			short isSaved, HttpServletRequest request, HttpServletResponse response) throws Exception {
		Shoppingcart cart = loadShoppingcartByUuid(cartUuid);
		Customer customer = (Customer) RequestContext.getCurrentUser();
		if (cart == null)
		{
		...
		ProductSku sku = productService.getProductSkuByProductSkuCode(productsku);
	...
	@Override
	public Shoppingcart addProductToCart4WS(Shoppingcart cart, ProductSku sku, Integer quantity, String accessoryIds, String customMade)
			throws Exception {
		ShoppingcartItem newCartItem;
		if (StringUtils.isBlank(customMade))
		{
	...
	public Shoppingcart addProductToCart(String cartUuid, String productsku, Integer quantity, String accessoryIds,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		return addProduct(cartUuid, productsku, quantity, accessoryIds, "", ShoppingcartItem.ISSAVED_NO, request, response);
	}

	public Shoppingcart addProductToCart(String cartUuid, String productsku, Integer quantity, String accessoryIds, String customMade,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public Shoppingcart addProductToFavorite(String cartUuid, String productsku, Integer quantity, String accessoryIds,
	...
	private ShoppingcartItem doAdd(Shoppingcart cart, ShoppingcartItem newItem, Integer quantity) throws Exception {
		if (newItem.getItemType().equals(Constants.ITEM_TYPE_PRODUCT))
		{// 增加产品类型的商品
		...
		else if (newItem.getItemType().equals(Constants.ITEM_TYPE_GC))
		{// 礼券类型的商品
			if (newItem.getShoppingcartItemId() == null
					|| (newItem.getShoppingcart() != null && !newItem.getShoppingcartItemId().equals(cart.getShoppingcartId())))
			{
				newItem.setShoppingcart(cart);
				cart.addShoppingCartItem(newItem);
			}
	...
	public Shoppingcart doDeleteItem(String cartUuid, String shoppingcartItemId, HttpServletRequest request, HttpServletResponse response) {
		...
		if (toDeleteItem != null)
		{
			...
			if (toDeleteItem.getParent() != null)
				toDeleteItem.getParent().getShoppingcartItems().remove(toDeleteItem);
	 ...
	 * 减一个商品
	 */
	public Shoppingcart doDeleteItemOne(String cartUuid, String shoppingcartItemId, HttpServletRequest request, HttpServletResponse response) {
		...
		if (toDeleteItem != null)
		{
			...
			try
			{
				toDeleteItem.setQuantity(toDeleteItem.getQuantity() - 1);
			}
			catch (Exception e)
			{

			}
			cart.addShoppingCartItem(toDeleteItem);

			System.out.println("减少了一个商品");
			// removePromotions(cart);
			refreshAndUpdateCart(cart);
			refreshAndUpdateCartCookie(cart, request, response);
		}
	 ...
	 * @throws Exception
	 */
	public Shoppingcart addQuantityOne(String cartUuid, Integer productSkuId, String accessoryIds, String customMade, short isSaved,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		Shoppingcart cart = loadShoppingcartByUuid(cartUuid);
		Customer customer = (Customer) RequestContext.getCurrentUser();
		if (cart == null)
		...
		ProductSku sku = productService.getProductSku(productSkuId);
		ShoppingcartItem newCartItem;
		if (StringUtils.isBlank(customMade))
		{
			newCartItem = shoppingcartItemManager.newShoppingcartItem(sku, 1, accessoryIds, isSaved);
		}
		else
		{
			newCartItem = shoppingcartItemManager.newShoppingcartItem(sku, 1, accessoryIds, customMade, isSaved);
		}

		System.out.println("增加了一个商品");
		doAdd(cart, newCartItem, 1);
		// refresh shoppingcart
		refreshAndUpdateCart(cart);
		// refresh cookie
		refreshAndUpdateCartCookie(cart, request, response);
	...
	public Shoppingcart doUniteShoppingcarts(String cartDBUuid, String cartCookieUuid, HttpServletRequest request,
		...
		else
		{
			...
			{// 合并,将匿名购物车中的物品放入到客户原来数据库中的购物车里
				...
				{// 此时一定是timeout的情况下才出现，此时直接将登陆用户的购物车返回即可
					refreshAndUpdateCart(cartDb);
					refreshAndUpdateCartCookie(cartDb, request, response);
				}
			}
	...
	public void doMoveCartItemToFavorite(String cartUuid, Integer itemId, HttpServletRequest request, HttpServletResponse response) {
		...
		if (itemf != null && isSameAccessories(itemf, itemc))
		{
			...
			en: for (ShoppingcartItem c : cItems)
			{// 改变该产品的子产品的数量
	...
	public boolean doMoveFavoriteItemToCart(String cartUuid, Integer itemId, HttpServletRequest request, HttpServletResponse response) {
		...
		if (itemc != null && isSameAccessories(itemf, itemc))
		{
			...
			en: for (ShoppingcartItem f : fItems)
			{// 改变该产品的子产品的数量(虚拟产品)
		...
		for (ShoppingcartItem item : cartItems)
		{
			...
			if (product.getProductSkuCode().equals(testItem.getProductSku().getProductSkuCode()))
			{
				...
				if ((p == null && testItem.getParent() == null)
						|| (p != null && testItem.getParent() != null && p.getProductSku().getProductSkuCode().equals(
								testItem.getParent().getProductSku().getProductSkuCode())))
				{
					// 检查附件是否相同
	 ...
	 * @return
	 */
	private boolean isExitsInFavorite(Shoppingcart cart, ShoppingcartItem testItem) {
	...
	/**
	 * 功能:在此处使用计费引擎重新计算购物车中价格
	 * <p>
	 * 作者 杨荣忠 2015-5-20 下午02:45:42
	 * 
	 * @param cart
	 */
	 ...
	 * 功能:重新刷新保存购物车cookie
	 * <p>
	 * 作者 杨荣忠 2015-5-20 下午02:46:19
	 * 
	 * @param cart
	 * @param request
	 * @param response
	 */
	private void refreshAndUpdateCartCookie(Shoppingcart cart, HttpServletRequest request, HttpServletResponse response) {
		if (cart != null)
		{
			RequestUtil.setCookie(response, com.cartmatic.estore.cart.CheckoutConstants.SHOPPINGCART_COOKIE, cart.getUuid(), request
					.getContextPath());
			RequestUtil.setCookie(response, com.cartmatic.estore.cart.CheckoutConstants.SHOPPINGCART_PRICE_COOKIE, cart.getTotal()
					.toString(), request.getContextPath());
			RequestUtil.setCookie(response, com.cartmatic.estore.cart.CheckoutConstants.C_ITEMCOUNT_COOKIE, String.valueOf(cart
					.getBuyNowItemsCount()), request.getContextPath());
			RequestUtil.setCookie(response, com.cartmatic.estore.cart.CheckoutConstants.F_ITEMCOUNT_COOKIE, String.valueOf(cart
					.getBuyLaterItemsCount()), request.getContextPath());
			RequestUtil.setCookie(response, com.cartmatic.estore.cart.CheckoutConstants.SHOPPINGCAT_ITEMCOUT_COOKIE, String.valueOf(cart
					.getItemsCount()), request.getContextPath());
		}

	}

	public void setShoppingcartItemManager(ShoppingcartItemManager shoppingcartItemManager) {
	...
	public boolean updateShoppingcartItem(String itemId, String skuNew, Integer newQuantity, HttpServletRequest request,
			HttpServletResponse response) {
		boolean temp = true;
		ShoppingcartItem item = shoppingcartItemManager.getById(Integer.valueOf(itemId));
		...
		if (item.getProductSku().getProductSkuCode().equals(skuNew))
		{// 更新数量
			Integer oldQu = item.getQuantity();
			item.setQuantity(newQuantity);
			temp = this.checkInventory(item, item.getProductSku().getProductSkuCode());
			if (!temp)
			{
		...
		{// TODO 更换新的商品了。此种情况为测试，需要shoppingart新的UI出来之后再进行测试、设计、修改。
			ProductSku productSku = productService.getProductSkuByProductSkuCode(skuNew);
	...
	private void removePromotions(Shoppingcart cart) {
		// cart.getShoppingCartPromotions().clear();
		// This is a bug which is always locking the shoppingcart_promotion
		// table.
		// Fixed on 2011-1-29 by Ryan.
		// this.shoppingcartPromotionManager.deleteShoppingcartPromotion(cart);
		if (cart.getIsUsedCoupon() != null && cart.getIsUsedCoupon() == Shoppingcart.ISUSECOUPON_YES)
		{
	...
	private boolean checkInventory(ShoppingcartItem item, String skuCode) {
		...
		// temp = CheckoutConstants.CANNOTMOVE_C_NOINVENTORY_NOMORE+":"+
		// item.getProductSku().getProduct().getProductName();
		// } else if (type == 3) {
		// temp = CheckoutConstants.CANNOTMOVE_C_NOINVENTORY+":"+
		// item.getProductSku().getProduct().getProductName();
		// }
		return temp;
	}
	...
	/**
	 * 卷控码验证，成功验证返回true，反之false
	 */
	public boolean doCheckJuankong(String number) {
		boolean flag = false;
		try
		{
			flag = UrlText.doCheckJuankong(number);
		}
		catch (IOException e)
		{
	...
	@Override
	public Shoppingcart addQuantityOne(String cartUuid, Integer productSkuId, String accessoryIds, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		// TODO Auto-generated method stub
		return addQuantityOne(cartUuid, productSkuId, accessoryIds, "", ShoppingcartItem.ISSAVED_NO, request, response);
	}
