...
public class OrderPickListController extends GenericController<OrderPickList>
{
	...
	public ModelAndView createPickList(HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView completeCreatePickList(HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public ModelAndView removeFromPickList(HttpServletRequest request, HttpServletResponse response) {
	 ...
	 * @return
	 */
	public ModelAndView validateShipment(HttpServletRequest request, HttpServletResponse response) throws Exception {
		AjaxView ajaxView = new AjaxView(response);
		...
		if (orderShipment != null)
		{
			...
			jmap.put("shippingAddress", oa.getState() + " " + oa.getCity() + " " + oa.getAddress1() + " "
					+ (oa.getAddress2() == null ? "" : oa.getAddress2()));
	...
	public ModelAndView completeShipping(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		OrderShipment orderShipment = salesOrderManager.getOrderShipmentByShipmentNo(shipmentNo);
		Integer salesOrderId = orderShipment.getSalesOrderId();
		SalesOrder salesOrder = orderService.getSalesOrderById(salesOrderId);
		// SalesOrder salesOrder2 = orderService.getById(salesOrderId);
		salesOrder.setPaidAmount(salesOrder.getTotalAmount());// 更新订单已付款金额
		...
		try
		{
			...
			for (Map<String, Integer> sku : originalSkus)
			{
				...
				{
					// 拆单
					Map<Integer, String> moveToMap = salesOrderManager.getShipments4MoveItemById(orderShipment.getSalesOrderId(),
							orderShipment.getOrderShipmentId());
			...
			orderProcessFlowManager.doCompletShipping(shipmentNo, carrierName, trackingNo, (AppUser) RequestContext.getCurrentUser(),
					shippingCostPaid);
			ajaxView.setStatus(new Short("1"));
	...
	public ModelAndView printPickList(HttpServletRequest request, HttpServletResponse response) {
	...
	/*
	 * 通过pdf打印 public ModelAndView printPackingSlip(HttpServletRequest request,
	 * HttpServletResponse response){
	 * 
	 * Map<String, Object> model = getInitModel(); updateModel(request, model);
	 * 
